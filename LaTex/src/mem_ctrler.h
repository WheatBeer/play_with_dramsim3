...
public:
    mem_ctrler_base_t(const std::string& m_config_file, 
                      const std::string& m_output_dir) 
    : clk(0) {
        mem = dramsim3::GetMemorySystem(m_config_file, m_output_dir,
                              std::bind(&mem_ctrler_base_t::read_callback, 
                                        this, std::placeholders::_1),
                              std::bind(&mem_ctrler_base_t::write_callback, 
                                        this, std::placeholders::_1));
    }
...
public:
    trace_mem_ctrler_t(const std::string& m_config_file, 
                       const std::string& m_output_dir,
...
public: 
    timing_mem_ctrler_t(const std::string& m_config_file, 
                        const std::string& m_output_dir)
    : mem_ctrler_base_t(m_config_file, m_output_dir), 
